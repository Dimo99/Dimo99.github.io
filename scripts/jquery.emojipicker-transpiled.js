...
;(function ($) {
  ...
  var categories = [{ name: 'people', label: 'People' }, { name: 'nature', label: 'Nature' }, { name: 'food', label: 'Food' }, { name: 'activity', label: 'Activities' }, { name: 'travel', label: 'Travel & Places' }, { name: 'object', label: 'Objects' }, { name: 'symbol', label: 'Symbols' }, { name: 'flag', label: 'Flags' }];
  ...
  function EmojiPicker(element, options) {
    ...
    var possiblePositions = ['left', 'right'
    /*,'top',
    'bottom'*/];
  ...
  $.extend(EmojiPicker.prototype, {
    ...
    addPickerIcon: function addPickerIcon() {
      ...
      if (this.settings.button) {
        ...
        this.$icon = $('<div class="emojiPickerIcon"></div>').height(iconHeight).width(iconHeight).addClass(this.settings.iconColor).css('backgroundColor', this.settings.iconBackgroundColor);
    ...
    listen: function listen() {
      ...
      this.$picker.find('nav .tab').click($.proxy(this.emojiCategoryClicked, this)).mouseover($.proxy(this.emojiTabMouseover, this)).mouseout($.proxy(this.emojiMouseout, this));
    ...
    emojiClicked: function emojiClicked(e) {
    ...
    emojiMouseover: function emojiMouseover(e) {
    ...
    emojiTabMouseover: function emojiTabMouseover(e) {
      var section = '';
      if ($(e.target).parent().hasClass('tab')) {
        section = $(e.target).parent().attr('data-tab');
      } else {
    ...
    pickerClicked: function pickerClicked(e) {
      e.stopPropagation();
    },
    ...
    searchCharEntered: function searchCharEntered(e) {
  ...
  function getPickerHTML() {
    ...
    for (var i = 0; i < categories_length; i++) {
      nodes.push('<div class="tab' + (!localStorageSupport && i == 0 ? ' active' : '') + '" data-tab="' + categories[i].name + '"><div class="emoji emoji-tab-' + categories[i].name + '"></div></div>');
    }
    nodes.push('</nav>');
